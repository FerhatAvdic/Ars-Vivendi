(function () {
    ...
    avApp.controller("sponsorsController", ['$scope', '$location', 'dataService', function ($scope, $location, dataService) {
            ...
            createItem: function (form) {
                ...
                dataService.create(crud.apiTarget, $scope.newItem, function (res) {
                    if (res.status === 200) {
                        ...
                        crud.listItems();
                        toastr.success("Uspješno dodan sponzor!");
                        //console.log(res.data);
                    }
                    else {
                        toastr.error("Greška prilikom dodavanja sponzora");
            ...
            updateItem: function (form) {
                ...
                dataService.update(crud.apiTarget, $scope.editingItem.id, $scope.editingItem, function (res) {
                    if (res.status === 200) {
                        ...
                        $scope.resetForm(form);
                        crud.listItems();
                        toastr.success("Uspješna izmjena sponzora");
                        //console.log(res.data);
                    }
                    else {
                        toastr.error("Greška prilikom izmjene sponzora");
            ...
            deleteItem: function (id) {
                dataService.remove(crud.apiTarget, id, function (res) {
                    if (res.status === 200) {
                        crud.listItems();
                        console.log(res.data);
                        toastr.success("Uspješno brisanje sponzora");
                    }
                    else {
                        toastr.error("Greška prilikom brisanja sponzora");
                        console.log('Error: ' + res.data);
                    }
                });
            }
        ...
        crud.object = sponsorObj;
        crud.apiTarget = "sponsors";
        $scope.crud = crud;
