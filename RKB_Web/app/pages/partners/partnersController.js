(function () {
    ...
    avApp.controller("partnersController", ['$scope', '$location', 'dataService', function ($scope, $location, dataService) {
            ...
            createItem: function (form) {
                ...
                dataService.create(crud.apiTarget, $scope.newItem, function (res) {
                    if (res.status === 200) {
                        ...
                        crud.listItems();
                        toastr.success("Uspješno dodan partner!");
                        //console.log(res.data);
                    }
                    else {
                        toastr.error("Greška prilikom dodavanja partnera");
            ...
            updateItem: function (form) {
                ...
                dataService.update(crud.apiTarget, $scope.editingItem.id, $scope.editingItem, function (res) {
                    if (res.status === 200) {
                        ...
                        $scope.resetForm(form);
                        crud.listItems();
                        toastr.success("Uspješna izmjena partnera");
                        //console.log(res.data);
                    }
                    else {
                        toastr.error("Greška prilikom izmjene partnera");
            ...
            deleteItem: function (id) {
                dataService.remove(crud.apiTarget, id, function (res) {
                    if (res.status === 200) {
                        crud.listItems();
                        console.log(res.data);
                        toastr.success("Uspješno brisanje partnera");
                    }
                    else {
                        toastr.error("Greška prilikom brisanja partnera");
                        console.log('Error: ' + res.data);
                    }
                });
            }
        ...
        crud.object = partnerObj;
        crud.apiTarget = "partners";
        $scope.crud = crud;
