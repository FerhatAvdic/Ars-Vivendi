(function () {
    ...
    avApp.controller("discountsController", ['$scope', '$location', 'dataService', function ($scope, $location, dataService) {
            ...
            createItem: function (form) {
                ...
                dataService.create(crud.apiTarget, $scope.newItem, function (res) {
                    if (res.status === 200) {
                        ...
                        crud.listItems();
                        toastr.success("Uspješno dodan bonitet!");
                        //console.log(res.data);
                    }
                    else {
                        toastr.error("Greška prilikom dodavanja boniteta");
                        console.log('Error: ' + res.data);
                    }
                    $('#newDiscountModal').modal('hide');
                });
            },
            updateItem: function (form) {
                ...
                dataService.update(crud.apiTarget, $scope.editingItem.id, $scope.editingItem, function (res) {
                    if (res.status === 200) {
                        ...
                        $scope.resetForm(form);
                        crud.listItems();
                        toastr.success("Uspješna izmjena boniteta");
                        //console.log(res.data);
                    }
                    else {
                        toastr.error("Greška prilikom izmjene boniteta");
            ...
            deleteItem: function (id) {
                dataService.remove(crud.apiTarget, id, function (res) {
                    if (res.status === 200) {
                        crud.listItems();
                        console.log(res.data);
                        toastr.success("Uspješno brisanje boniteta");
                    }
                    else {
                        toastr.error("Greška prilikom brisanja boniteta");
                        console.log('Error: ' + res.data);
                    }
                });
            }
        ...
        crud.object = discountObj;
        crud.apiTarget = "discounts";
        $scope.crud = crud;
